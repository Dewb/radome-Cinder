...
vec2 getUV() {
    ...
    if (mappingMode == 0) {
        ...
        // Mirrored quadrants
        normalUV = vec2(abs(position.x)/domeRadius,
                        4.0 * asin(position.y/domeHeight)/(2.0*3.141592));
...
void main()
{
    ...
    if (position.y < 0.1) {
        ...
        if (distSquared < pow(domeRadius, 2.0) ) { // unless it's under the dome
            discard;
        } else {
            gl_FragColor = vec4(0.72, 0.15, 0.1, 1.0);
        }
    } else {
        ...
        if (videoMix >= 0.0)
        {
            ...
            vec2 uv = getUV();
            vec4 videoColor = vec4(1.0, 0.0, 0.0, 1.0); //texture2D(video, uv);
